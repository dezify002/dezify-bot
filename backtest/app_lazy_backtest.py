...
# Lazy import backtest engine - will import when route is called
BACKTEST_AVAILABLE = True  # Assume available, check at runtime
BacktestEngine = None

def _get_backtest_engine():
    """Lazy import backtest engine to catch errors at runtime."""
    ...
    if BacktestEngine is None:
        try:
            ...
            print(f"⚠️ Backtest engine import failed: {error_msg}")
            raise RuntimeError(f"Backtest engine import failed: {error_msg}")
